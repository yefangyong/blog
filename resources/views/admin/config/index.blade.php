...
<script>
    function changeOrder(obj,conf_id) {
        var conforder = $(obj).val();
        $.post("{{url('admin/config/changeOrder')}}",{'_token':'{{csrf_token()}}','conf_id':conf_id,'conforder':conforder},function(data){
           if(data.status == 0) {
...
<script>
    function delCate($conf_id) {
        layer.confirm('您确定要删除这个导航吗？', {
            ...
            $.post('{{url('admin/config')}}/'+$conf_id,{'_method':'delete','conf_id':$conf_id,'_token':'{{csrf_token()}}'},function(data){
                if(data.status == 0) {
