...
<script>
    function changeOrder(obj,link_id) {
        var linkorder = $(obj).val();
        $.post("{{url('admin/links/changeOrder')}}",{'_token':'{{csrf_token()}}','link_id':link_id,'linkorder':linkorder},function(data){
           if(data.status == 0) {
...
<script>
    function delCate($link_id) {
        layer.confirm('您确定要删除这个文章吗？', {
            ...
            $.post('{{url('admin/links')}}/'+$link_id,{'_method':'delete','link_id':$link_id,'_token':'{{csrf_token()}}'},function(data){
                if(data.status == 0) {
